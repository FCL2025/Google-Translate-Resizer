...
Width changes are applied live while you drag the sliders. When the browser window becomes narrower, the panels scale down responsively while keeping your chosen left/right ratio.
...
```text
Google-Translate-Resizer-v0.1.13.zip
```
...
1. Download `Google-Translate-Resizer-v0.1.13.zip` from the latest release.
...
拖曳拉桿時會即時套用寬度變更。當瀏覽器視窗變窄時，左右區塊會依照你設定的比例自動縮小。
...
1. 從最新 Release 下載 `Google-Translate-Resizer-v0.1.13.zip`。
2. 解壓縮 ZIP。
